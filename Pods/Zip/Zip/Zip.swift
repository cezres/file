...
public class Zip {
    ...
    public class func unzipFile(zipFilePath: NSURL, destination: NSURL, overwrite: Bool, password: String?, progress: ((progress: Double) -> ())?) throws {
        ...
        if fileManager.fileExists(atPath: path) == false || fileExtensionIsInvalid(fileExtension: zipFilePath.pathExtension) {
            throw ZipError.FileNotFound
        }
...
//        var buffer = Array<CUnsignedChar>(count: Int(bufferSize), repeatedValue: 0)
        ...
        let key = FileAttributeKey.size.rawValue
        if let attributeFileSize = fileAttributes[key] as? Double {
...
//                    fileManager.createDirectory(atPath: <#T##String#>, withIntermediateDirectories: <#T##Bool#>, attributes: <#T##[String : AnyObject]?#>)
...
//                    try fileManager.createDirectory(parentDirectory, withIntermediateDirectories: true, attributes: directoryAttributes)
                }
            } catch {}
...
//            filePointer
    ...
    public class func zipFiles(paths: [NSURL], zipFilePath: NSURL, password: String?, progress: ((progress: Double) -> ())?) throws {
        ...
        // Process zip paths
        let processedPaths = ZipUtilities().processZipPaths(paths: paths)
        ...
        for path in processedPaths {
            ...
            if !isDirectory {
                ...
                do {
                    let fileAttributes = try fileManager.attributesOfItem(atPath: filePath)
                    if let fileDate = fileAttributes[FileAttributeKey.modificationDate.rawValue] as? NSDate {
                        let components = Calendar.current().components([.year, .month, .day, .hour, .minute, .second], from: fileDate as Date)
    ...
    internal class func fileExtensionIsInvalid(fileExtension: String?) -> Bool {
        ...
        return !isValidFileExtension(fileExtension: fileExtension)
    }
