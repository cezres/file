...
extension UIImage {
    class func blurEffect(cgImage: CGImage, boxSize: CGFloat) -> UIImage! {
        return UIImage(cgImage: cgImage.blurEffect(boxSize: boxSize))
    }
    
    func blurEffect(boxSize: CGFloat) -> UIImage! {
        return UIImage(cgImage: bluredCGImage(boxSize: boxSize))
    }
    
    func bluredCGImage(boxSize: CGFloat) -> CGImage! {
        return cgImage!.blurEffect(boxSize: boxSize)
    }
}

extension CGImage {
    func blurEffect(boxSize: CGFloat) -> CGImage! {
        
        let boxSize = boxSize - CGFloat(Int(boxSize) % 2) + 1
