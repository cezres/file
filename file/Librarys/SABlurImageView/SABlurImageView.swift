...
public extension SABlurImageView {
    ...
    public func configrationForBlurAnimation(boxSize: CGFloat = 100) {
        ...
        (0..<type(of: self).MaxImageCount).forEach {
            ...
            image = image.blurEffect(boxSize: boxSize)
            if let cgImage = image.cgImage {
                cgImages += [cgImage]
            }
        }
...
public extension SABlurImageView {
    public func addBlurEffect(boxSize: CGFloat, times: UInt = 1) {
        ...
        self.image = addBlurEffectTo(image: image, boxSize: boxSize, remainTimes: times)
    }
    
    private func addBlurEffectTo(image: UIImage, boxSize: CGFloat, remainTimes: UInt) -> UIImage {
        return remainTimes > 0 ? addBlurEffectTo(image: image.blurEffect(boxSize: boxSize), boxSize: boxSize, remainTimes: remainTimes - 1) : image
    }
}
...
public extension SABlurImageView {
    public func blur(percentage: CGFloat) {
        ...
        if previousPercentage - percentage  > 0 {
            ...
            if index > 0 {
                setLayers(index: index, percentage: percentage, currentIndex: index - 1, nextIndex: index)
            }
        } else {
            let index = Int(floor(percentage * 10))
            if index < cgImages.count - 1 {
                setLayers(index: index, percentage: percentage, currentIndex: index, nextIndex: index + 1)
            }
        }
    ...
    private func setLayers(index: Int, percentage: CGFloat, currentIndex: Int, nextIndex: Int) {
        if index != previousImageIndex {
            CATransaction.animationWithDuration(duration: 0) { layer.contents = self.cgImages[currentIndex] }
            ...
            CATransaction.animationWithDuration(duration: 0) {
        ...
        CATransaction.animationWithDuration(duration: 0) { self.nextBlurLayer?.opacity = Float(alpha) }
    }
}
    ...
    public func startBlurAnimation(duration: TimeInterval) {
        ...
        animations = cgImages.map { cgImage in
            return { [weak self] in
                ...
                self?.layer.add(transition, forKey: type(of: self!).FadeAnimationKey)
